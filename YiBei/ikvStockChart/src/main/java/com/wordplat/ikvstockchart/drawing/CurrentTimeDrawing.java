...
public class CurrentTimeDrawing implements IDrawing {
    ...
    public CurrentTimeDrawing(int digits) {
        this.digits = digits;
    }
...
//        canvas.clipRect(kLineRect);
        ...
        if (maxIndex > 0) {
            ...
            entrys = entrySet.getEntryList().get(entrySize - 1);
     ...
     * @param yCoord
     */
    private void drawCurrentStateMap(Canvas canvas, double text, float yCoord) {
//        yCoord = yCoord + 20;//为了好看，做整体向下平易操作
        PathEffect effects = new DashPathEffect(new float[]{5, 5, 5, 5}, 1);
        linePaint.setPathEffect(effects);
        Path path1 = new Path();
        path1.moveTo(kLineRect.left, yCoord);//替换0-->(rectWidth - 200)
